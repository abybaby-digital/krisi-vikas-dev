...
class FertilizerImageScreen extends StatefulWidget {
  const FertilizerImageScreen({Key? key}) : super(key: key);

  @override
  State<FertilizerImageScreen> createState() => _FertilizerImageScreenState();
}

class _FertilizerImageScreenState extends State<FertilizerImageScreen> {
  bool showCPI = false;
          ...
          InkWell(
            onTap: () async {
              ...
              if (images[0] == "") {
                ...
                if (images[1] != "") {
                  second_image = await MultipartFile.fromFile(
                    images[1],
                    filename: images[1].split("/").last,
                  );
                }
                if (images[2] != "") {
                  third_image = await MultipartFile.fromFile(
                    images[2],
                ...
                FormData formData = FormData.fromMap({
                  ...
                  "category_id": FertilizerData.categoryId ?? 9,
                  "description": FertilizerData.description,
                  "image1": first_image,
                  "image2": second_image,
                  "image3": third_image,
                  "price": FertilizerData.price ?? 0,
                  "is_negotiable": FertilizerData.IsNegotiable,
                  "country_id": FertilizerData.countryId ?? 1,
                  "state_id": FertilizerData.stateId ?? 36,
                  "city_id": FertilizerData.cityId ?? 1,
                  "district_id": FertilizerData.districtId ?? 1234,
                  "latlong": "${FertilizerData.lat},${FertilizerData.long}",
                  "pincode": FertilizerData.zipcode
                });
  ...
  doThisONLaunch() async {
    ...
    CategoryIdWhilePosting = FertilizerData.categoryId.toString();
  }

  getApiUrlByCategoryId() {
      url = baseUrl + fertilizerAddUrl;
  }
