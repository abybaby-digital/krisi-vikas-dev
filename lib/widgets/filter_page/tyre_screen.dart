...
import 'package:krishivikas/widgets/filter_page/tyres_filter_screen.dart';
...
class TyreScreen extends StatefulWidget {
  ...
  const TyreScreen({Key? key, required this.type, required this.currentTabType, required this.categoryId})
      : super(key: key);
...
class _TyreScreenState extends State<TyreScreen> {
  ...
  void initState() {
    super.initState();
    TyreFilterData(brandId, districtId, stateId, priceStart, priceEnd, byYear);
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Text('${widget.currentTabType} ${widget.type.toString()}',
                    style: TextStyle(
                      fontSize: 18,
                      color: Colors.white,
  ...
  Future<List<Tyre>?> TyreFilterData(String brandId,String districtId,String stateId,String priceStart,String priceEnd,String byYear) async {
    Map data = {"user_id": SharedPreferencesFunctions.userId,"user_token":SharedPreferencesFunctions.token,"type":condition,"price_start":priceStart,"price_end":priceEnd,"skip":"0","take":"100","purchase_year":byYear,"model":"","brand":brandId,"district":districtId,"state":stateId};
    String body = json.encode(data);
    // logger.e(body);
    final response = await http.post(
        Uri.parse(baseUrl + tyresFilterUrl),
        headers: {
    ...
    if (response.statusCode == 200) {
      ...
      final List<Tyre> ListTyreData =  TyreFilterModal.fromJson(responseData).data.toList();
      listData = ListTyreData;
